...
async function salvarPerfil() {
    ...
    const fotoInput = document.getElementById('inFotoP');
...
async function marcarComoTrocado(idDB) {
    if (confirm("Marcar anúncio como concluído? Ele não aparecerá mais no feed.")) {
...
async function excluirProduto(idDB) {
    if (confirm("Tem certeza que deseja excluir este anúncio permanentemente?")) {
        await db.collection('produtos').doc(idDB).delete();
    }
}
...
function criarCardHTML(p) {
    ...
    const estaConcluido = p.status === "trocado";
    
    let iconeTipo = "🔄";
    let textoBotao = "Trocar";
    if (p.tipoNegocio === "Leilão") { iconeTipo = "🔥"; textoBotao = "Dar Lance"; }
    else if (p.tipoNegocio === "Doação") { iconeTipo = "🎁"; textoBotao = "Quero Presente"; }
    else if (p.tipoNegocio === "Serviço") { iconeTipo = "💼"; textoBotao = "Contratar"; }

    return `
        <div class="conteiner_produto" style="${estaConcluido ? 'opacity: 0.6;' : ''}">
            <div class="produto_imgEdescri" onclick="window.location.href='produto.html?id=${p.id}'">
                ...
                <div class="descricao_produto">
                    <span style="font-size:10px; background:#eee; padding:2px 6px; border-radius:4px;">
                        ${iconeTipo} ${p.tipoNegocio || 'Troca'} | ${p.categoria}
                    </span>
                    <h2 class="nome_produto">${p.nome} ${estaConcluido ? '<small>(CONCLUÍDO)</small>' : ''}</h2>
                    <p class="paragrafo_produto">${p.descricao.substring(0, 45)}...</p>
                    
                    ${ehMeu ? `
                        <div style="display:flex; gap:10px; margin-top:10px;">
                            <button style="border:1px solid #ddd; padding:5px 10px; border-radius:5px; background:white; cursor:pointer;" onclick="event.stopPropagation(); abrirModalEdicaoAnuncio('${p.idDB}')">✏️ Editar</button>
                            ${!estaConcluido ? `<button style="border:1px solid #ddd; padding:5px 10px; border-radius:5px; background:white; cursor:pointer;" onclick="event.stopPropagation(); marcarComoTrocado('${p.idDB}')">✔️ Concluir</button>` : ''}
                            <button style="border:1px solid #ddd; padding:5px 10px; border-radius:5px; background:white; cursor:pointer;" onclick="event.stopPropagation(); excluirProduto('${p.idDB}')">🗑️ Excluir</button>
                        </div>
                    ` : ''}
                </div>
            </div>
            <div class="conteiner_usuarioEbotao_trocar">
                ...
                <div class="conteiner_botao">
                    ${ehMeu ? `<span style="font-size:12px; color:#999;">Seu Anúncio</span>` 
                    : `<button class="botao_trocar" onclick="event.stopPropagation(); window.location.href='produto.html?id=${p.id}'">${textoBotao}</button>`}
                </div>
            </div>
        </div>`;
}
...
function renderizarProdutos(filtro = null) {
    ...
    let exibir = produtosGlobaisNaNuvem.filter(p => p.status !== "trocado" || abaAtual === 'meusAnuncios');

    if (abaAtual === 'meusAnuncios') {
        exibir = produtosGlobaisNaNuvem.filter(p => p.donoEmail === usuarioLogado.email);
    } else if (abaAtual === 'favoritos') {
        const favs = JSON.parse(localStorage.getItem('meusFavoritos')) || [];
        exibir = produtosGlobaisNaNuvem.filter(p => favs.includes(String(p.id)));
    } else if (['Troca', 'Leilão', 'Doação', 'Serviço'].includes(abaAtual)) {
        exibir = exibir.filter(p => p.tipoNegocio === abaAtual);
    } else if (abaAtual !== 'tudo' && abaAtual !== 'Ver Tudo') {
        exibir = exibir.filter(p => p.categoria === abaAtual);
    }
...
function mostrarMeusAnuncios() { abaAtual = 'meusAnuncios'; renderizarProdutos(); fecharTudo(); }
